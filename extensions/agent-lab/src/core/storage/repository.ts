...
export class CoreRepository {
  ...
  private _applyCoreMigrations(): void {
    ...
    // Add memory_spec / endowment columns to lab_agent_instances if missing
    // (N-I9，spec §2.2 step 5：装配层注册持久化 memorySpec/endowment——
    // 旧表无此列，经 ALTER 新增可空列；新库由 CORE_SCHEMA 直接建列）
    for (const col of ["memory_spec", "endowment"]) {
      const cols = this.db.prepare(
        `PRAGMA table_info(lab_agent_instances)`
      ).all() as Array<{ name: string }>;
      if (!cols.some((c) => c.name === col)) {
