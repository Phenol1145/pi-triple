...
const HOME = homedir();
const KEYS = ["AGENT_LAB_CONFIG_DIR", "AGENT_LAB_DB_PATH", "PI_TRIPLE_HOME", "PI_TENANT"];
...
test("localConfigDir: AGENT_LAB_CONFIG_DIR 最高优先", () => {
  ...
  process.env.PI_TENANT = "t1";
  assert.equal(localConfigDir(), "/x/cfg");
});

test("localConfigDir: 有 PI_TENANT → pitHome/data/pi-config/<t>/agent-lab", () => {
  delete process.env.AGENT_LAB_CONFIG_DIR;
  process.env.PI_TENANT = "uuid-1";
...
test("sharedDbPath: 有 PI_TENANT → pitHome/data/shared/agent-lab/agent-lab.db", () => {
