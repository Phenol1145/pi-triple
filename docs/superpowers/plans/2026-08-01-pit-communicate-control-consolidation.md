# ptl-communicate / ptl-control 整合实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 tmux 会话管理收敛到 ptl-control、抽共享模块 `_shared/`、打通会话身份（tmux 名 ↔ sessionId ↔ registry/presence）、实现 name 持久化。

**Architecture:** 新建 `extensions/_shared/`（tmux-session.ts / registry.ts / presence.ts / paths.ts，无 index.ts 避免被 pi 误加载），ptl-communicate 与 ptl-control 均改为 import 共享模块；communicate 删除 tmux 三命令与本地 registry/presence 副本；control 用共享模块重构并增强（自动命名、tmux env 回写、ls 在线状态、name 持久化）。

**Tech Stack:** TypeScript 5.7（`.ts` 源码直接运行，无 build）、vitest 3、node:child_process / node:crypto、tmux CLI。

**Spec:** `docs/superpowers/specs/2026-08-01-ptl-communicate-control-consolidation-design.md`
...
- 平台命令 `ptl update --all` 已修复可用（bundled 同步）；`syncBundledExtensions` 的 `cpSync(force)` **不会删除目标多余文件**——communicate 删除的文件需手动从共享层清理
...
- Create: `extensions/_shared/registry.ts`（内容 = 现 `extensions/ptl-communicate/registry.ts` 原样，仅改头部注释）
- Create: `extensions/_shared/presence.ts`（内容 = 现 `extensions/ptl-communicate/presence.ts` + 新增静态方法 `updateName`）
- Delete: `extensions/ptl-communicate/registry.ts`、`extensions/ptl-communicate/presence.ts`
- Modify: `extensions/ptl-communicate/index.ts`（import 路径改 `../_shared/...`）
...
git mv extensions/ptl-communicate/registry.ts extensions/_shared/registry.ts
git mv extensions/ptl-communicate/presence.ts extensions/_shared/presence.ts
```
...
```ts
  /**
   * 一次性静态更新 state.json 的 name 字段（供 ptl-control 使用，不启动心跳）。
...
`extensions/ptl-communicate/index.ts` 顶部两处：
...
```ts
import { Presence } from "../../extensions/ptl-communicate/presence.js";
import { Registry } from "../../extensions/ptl-communicate/registry.js";
```
...
git add -A extensions/_shared extensions/ptl-communicate/index.ts test/unit/intercom.test.ts
...
- Produces:
  ...
  - 所有 `name` 参数均为**去 `ptl-` 前缀**的会话名；内部拼 `ptl-<name>`
...
describe("TmuxSession", () => {
  ...
  it("listPitSessions: 过滤 ptl- 前缀并去前缀", () => {
    const calls: string[][] = [];
    const t = new TmuxSession(fakeRunner([
      { match: (a) => a[0] === "list-sessions", stdout: "ptl-a\nptl-b\nother\n0\n" },
    ], calls));
  ...
  it("listSessionsDetail: 解析 name/windows/created", () => {
    const calls: string[][] = [];
    const t = new TmuxSession(fakeRunner([
      { match: (a) => a[0] === "list-sessions", stdout: "ptl-x 3 1700000000\nptl-y 1 1700000100\n" },
    ], calls));
  ...
  it("startSession: 固定名参数组装（-d -s -x -y -e ... -- pi）", () => {
    ...
    expect(args[args.indexOf("-s") + 1]).toBe("ptl-agent1");
  ...
  it("startSession: 无 name 自动生成 auto-xxxxxx（冲突重试）", () => {
    ...
    expect(args[args.indexOf("-s") + 1]).toBe(`ptl-${r.name}`);
  });

  it("stopSession: kill-session -t =ptl-<name> 精确匹配", () => {
    const calls: string[][] = [];
    const t = new TmuxSession(fakeRunner([{ match: (a) => a[0] === "kill-session" }], calls));
    expect(t.stopSession("agent1")).toBe(true);
    const args = calls[0]!;
    expect(args).toEqual(["kill-session", "-t", "=ptl-agent1"]);
  });
  ...
  it("switchTo / detach / currentSessionName / env 读写", () => {
    ...
    const t = new TmuxSession(fakeRunner([
      ...
      { match: (a) => a[0] === "display-message", stdout: "ptl-mine\n" },
    ...
    expect(calls[0]).toEqual(["switch-client", "-t", "=ptl-agent1"]);
    expect(t.detach()).toBe(true);
    expect(t.currentSessionName()).toBe("ptl-mine");
    expect(t.setSessionEnv("mine", "PI_SESSION_ID", "019fabc123")).toBe(true);
    expect(calls[3]).toEqual(["set-environment", "-t", "ptl-mine", "PI_SESSION_ID", "019fabc123"]);
    expect(t.getSessionEnv("mine", "PI_SESSION_ID")).toBe("019fabc123");
    expect(calls[4]).toEqual(["show-environment", "-t", "ptl-mine", "PI_SESSION_ID"]);
  });
 ...
 * 会话名统一为"去 ptl- 前缀"的短名；tmux 实际名 = `ptl-<name>`。
 */
...
export class TmuxSession {
  ...
  sessionExists(name: string): boolean {
    return this.runner(["has-session", "-t", `ptl-${name}`]).status === 0;
  }
  ...
  private launch(name: string, env?: Record<string, string>): { ok: boolean; name: string; error?: string } {
    const args = ["new-session", "-d", "-s", `ptl-${name}`, "-x", "200", "-y", "50"];
  ...
  stopSession(name: string): boolean {
    return this.runner(["kill-session", "-t", `=ptl-${name}`]).status === 0;
  }

  switchTo(name: string): boolean {
    return this.runner(["switch-client", "-t", `=ptl-${name}`]).status === 0;
  }
  ...
  /** 当前 tmux 会话名（原始名，含 ptl- 前缀或任意其他会话）；非 tmux 内返回 null */
  ...
  setSessionEnv(name: string, key: string, value: string): boolean {
    return this.runner(["set-environment", "-t", `ptl-${name}`, key, value]).status === 0;
  }

  getSessionEnv(name: string, key: string): string | null {
    const r = this.runner(["show-environment", "-t", `ptl-${name}`, key]);
...
### Task 3: ptl-communicate 重构 — 删除 tmux 三命令 + 名字初始化

**Files:**
- Modify: `extensions/ptl-communicate/index.ts`
...
`extensions/ptl-communicate/index.ts` 中，将：
...
- [ ] **Step 2: 删除 `/ptl start|stop|sessions` 三个 handler 分支**
...
handler 末尾的 help 文本删除三行（`/ptl sessions ...`、`/ptl start ...`、`/ptl stop ...`），并追加一行提示：

```ts
  "  /ptl ps                   List registered sessions\n" +
  "  /ptl status               Intercom status\n" +
  "\nSession management: /control start|stop|ls (ptl-control)\n" +
...
grep -n "execSync\|/ptl start\|/ptl stop\|/ptl sessions" extensions/ptl-communicate/index.ts
```
...
```bash
git add extensions/ptl-communicate/index.ts
git commit -m "refactor(ptl-communicate): 删除 tmux 三命令（移交 ptl-control），name 初始化支持 PI_SESSION_NAME 与 registry 恢复"
```

---

### Task 4: ptl-control 重构 — 共享模块 + 增强命令面

**Files:**
- Modify: `extensions/ptl-control/index.ts`
...
`extensions/ptl-control/index.ts` 中删除本地 `hasTmux` / `tmuxName` / `listPitSessions` / `sessionExists` 四个函数与 `spawnSync`/`randomUUID` 之外的逻辑，替换为：
...
```ts
      if (cmd === "status") {
        ...
        if (tmux.hasTmux()) {
          ...
          lines.push(`  running: ${pits.length} ptl session(s)`);
...
grep -n "spawnSync\|hasTmux\|sessionExists\|listPitSessions" extensions/ptl-control/index.ts
```
...
```bash
git add extensions/ptl-control/index.ts
git commit -m "feat(ptl-control): 共享模块重构 — start 自动命名/PI_SESSION_NAME、ls 在线状态、name 持久化、tmux env 自注册"
```
...
- [ ] **Step 2: 同步共享层（ptl update --all）+ 清理残留**

```bash
node dist/ptl/pit.js update --all 2>&1 | grep -E '✅|❌'
# cpSync(force) 不会删除目标多余文件 —— 手动清理 communicate 已删除的副本：
rm -f /Users/anzhize/.pi-triple/data/shared/extensions/ptl-communicate/registry.ts
rm -f /Users/anzhize/.pi-triple/data/shared/extensions/ptl-communicate/presence.ts
```
...
```bash
cd /Users/anzhize/pi-platform
for e in _shared ptl-communicate ptl-control; do
...
import('file:///Users/anzhize/.pi-triple/data/shared/extensions/ptl-control/index.ts').then(m => console.log('ptl-control export:', typeof m.default));
import('file:///Users/anzhize/.pi-triple/data/shared/extensions/ptl-communicate/index.ts').then(m => console.log('ptl-communicate export:', typeof m.default));
"
```

预期：三行均输出 ok / function（ptl-communicate 走 ESM，default 为 function）。

- [ ] **Step 5: `/reload` 手动验证清单（当前会话或新会话）**

在 pi 会话中执行 `/reload` 后（或新开 `ptl start` 会话）逐项验证：

1. `/ptl help` → 无 start/stop/sessions，含"Session management: /control ..."提示
2. `/ptl start x` → 提示未知子命令或 usage（不执行任何 tmux 操作）
...
6. `/control start agent1` → `ptl-agent1` 固定名；`/ptl ps` 能看到 `agent1`（PI_SESSION_NAME 打通）
7. 与旧会话（如本会话）互不影响：`/ptl ps` 仍列出所有 registry 会话
