...
MemoryEntry {
  ...
  meta: {
    ...
    sourceTraces: Array<{ traceId, transitionSeq }>,   // 多源溯源（数组，一条经验可综合多次观察/通讯）
...
3. **版本化 CAS**——更新 = 新版本（不原地改）；**版本链为 DAG**（公域 fork 可分叉）；回写声明 base version，冲突检测（基线过期即拒绝，要求重新 fork）
...
```
δ 观察 → write-behind 观察缓冲（本地、checkpoint 持久化——崩溃不丢）
...
- **校验失败三级处置**：返回错误（定位到生产式）→ 草稿区（`unverified` 标记，检索默认排除）→ 反馈重试（≤2 次上限）
- **草稿区 TTL 7 天** + 自动归档（防无界增长）
...
**审核链（v1 完整矩阵，用户裁决）**：
- agent 侧策略（可配置）：全员投票 / veto 制 / 评审代表
- operator 侧策略（可配置）：agent 代审 / 一致放行 / 亲自审
- **quorum + 超时弃权**：离线 agent = 弃权（不阻塞）；审核请求聚合批量投递（防 O(N²) 洪峰）
...
- **DSP 可重放性（用户裁决 A）**：**检索结果快照纳入 checkpoint**——恢复时用快照而非重新检索（MemoryStore 演化不破坏确定性）；代价：checkpoint 体积增加（检索摘要量级，可接受）
- **预算**：DSP 有超预算截断策略（投影/检索摘要超限 → 截断 + 降级提示）
...
- **幂等**：接收侧按 msgId 去重（pit-communicate processedIds 是内存态——重启后重复投递，接收侧必须幂等）
- **mode 映射**：agent↔agent 消息**必须 auto 模式**（manual 模式会被人力门卡死"近实时"）
- **身份映射**：agentId → (tenantId, sessionId) 映射层（mailbox 按 tenant/session 寻址——需新建）
- **大小上限**：tapeFragment 上限（防撑爆接收方 DSP 预算）；flow human 门 = comms 等待点（带超时/取消语义）
...
4. 版本 DAG + base version 冲突检测（公域回写）
5. 沉淀先落库后事件；写入顺序条目先索引后；启动索引重建
6. 草稿区 TTL + 重试上限（烧钱封顶）
7. comms 幂等（msgId 去重）+ auto mode + 大小上限
8. DSP 检索快照进 checkpoint（可重放性）
9. 规则更新事务（EBNF + 编译产物同版本原子生效）
10. 审核 quorum/超时弃权；meta 规则修改 operator 亲审
