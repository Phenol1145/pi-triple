...
- **DSP 可重放性（用户裁决 A）**：**检索结果快照纳入 checkpoint**——恢复时用快照而非重新检索（MemoryStore 演化不破坏确定性）；快照格式 = 检索摘要文本 + 记忆版本号（轻量）；**默认上限 4KB/轮**（可配置，内容寻址去重）；恢复快照**不重检索不计数**（hitCount 以实时检索为准——快照恢复期间计数漂移，文档化接受）
