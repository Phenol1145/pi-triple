...
| 14 | 执行层形态 | batch = 进程单元；全角色 worker ×1（v1）；弹性加减 batch；动态构成留 v2 |
...
| 20 | agent-lab 归位 | 选择性迁移（taskpool/sorter/ingest/memory 保留迁 pg）；废弃代码先不删 |
| 21 | 执行路径 | 全部收敛到 PTH kernel；SDK 会话仅意图层；workloop 废弃（代码保留） |
| 22 | 代码落点 | src/pth/（原生模块） |
| 23 | 语言 v1 | TS + bash + Python 三解释器 |
