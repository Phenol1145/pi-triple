...
> 文档性质：已定稿的分层指标设计。
>
> 实施映射：ResourceProvider、Kernel/LLM/任务/Refine/记忆指标骨架和 batch IPC 指标接线已经存在；网络字节、部分生命周期与快照指标未观察到完整实现。
>
> 阅读关系：本文描述指标目标全集；“完全无计量”等现状判断是设计形成时的历史快照。参见[Kernel 设计综合总览](./2026-08-09-pth-kernel-design-synthesis.md)。
