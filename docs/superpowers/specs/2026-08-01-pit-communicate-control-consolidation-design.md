# ptl-communicate / ptl-control 整合设计

> 日期：2026-08-01
> 状态：已确认（用户逐节审阅通过）
> 范围：extensions/ptl-communicate · extensions/ptl-control · extensions/_shared（新增）

## 背景

两个内置扩展存在三类问题：

1. **tmux 会话管理职责重复且行为不一致**：`/ptl start/stop/sessions`（ptl-communicate 内）用 shell 字符串拼接 tmux 命令（违反平台"tmux 传参用 `-e` flag、禁 shell 拼接"硬约束），且不传模板环境变量；`/control start/stop/ls`（ptl-control）用 `spawnSync` + `-e` flag 正确实现。两套命令看到的"会话宇宙"不同。
2. **会话身份隔离**：ptl-communicate 有 registry/presence（在线/名称/模型），ptl-control 完全不感知，`/ptl ps` 与 `/control ls` 互相不知道对方。
3. **ptl-control 无状态持久化**：`/control name` 只改内存，重启即丢。

前置任务（已完成）修复了平台机制：`resolveBundledDir`（bundled 扩展定位，发布/开发双布局）、`ptl update --all` 真实可用、`ptl-communicate` 补 `"type": "module"`、共享层与仓库零差异。
...
├── registry.ts                       ← 从 ptl-communicate 迁入（会话注册表）
└── presence.ts                       ← 从 ptl-communicate 迁入（心跳/在线状态）

extensions/ptl-control/
└── index.ts                          ← import 共享模块；/control 命令面增强
extensions/ptl-communicate/
...
- `syncBundledExtensions`（`ptl update --all`）照常同步，零平台机制改动
- pi 扩展自动发现只匹配顶层 `*.ts` 与 `*/index.ts`；约束 `_shared/` 内**不放 index.ts** → 不会被误加载

**依赖方向**：`ptl-control → _shared`，`ptl-communicate → _shared`；`_shared` 零依赖（纯文件逻辑，无 pi API）。
...
| 名字 | 来源 | 可变性 |
|---|---|---|
| tmux 会话名 `ptl-<x>` | `/control start [name]` 时定 | 不可变（tmux 引用依赖） |
| registry name | communicate 启动时定 | 可变（`/ptl name`、`/control name`） |
| sessionId | pi 启动时生成 | 永不变 |

### tmux 会话名 `<x>` 的确定

- `/control start my-agent` → 固定名：tmux 会话 = `ptl-my-agent`，`PI_SESSION_NAME=my-agent`
- `/control start`（无参数）→ 自动名：生成 `auto-<6位 base36 随机>`（如 `auto-x8k2p1`），tmux 会话 = `ptl-auto-x8k2p1`；唯一性：`tmux has-session` 检查，冲突重试（最多 3 次，仍冲突则报错提示手输名字）
...
反向：/control ls 时
  → tmux show-environment -t ptl-<x> PI_SESSION_ID   拿 sessionId
  → 读 data/mailbox/{tenant}/{sessionId}/state.json   显示在线/模型/模式/名字
  → 查不到映射（新会话启动数秒窗口内）→ 降级为仅 tmux 基本信息，不报错
```

选型对比：映射文件方案（mailbox/{tenant}/tmux-map.json）引入并发写问题；registry 按 name 匹配会在 `/ptl name` 改名后失配。tmux env 方案对改名免疫、零文件、无并发。
...
- 删除 `/ptl start`、`/ptl stop`、`/ptl sessions`（tmux 职责移交 control；顺带移除其 shell 拼接隐患）
- 保留 `/ptl ps`（registry 视角）及 send/ask/share/broadcast/inbox/accept/reject/mode/name/status
...
| 测试 | 内容 |
|---|---|
| `test/unit/ext-shared-tmux.test.ts`（新） | tmux-session.ts 全逻辑，fake runner 注入（零真实 tmux）：`ptl-` 过滤；startSession 参数组装与 `-e`/`-- pi`；消毒；自动命名与唯一性重试；stopSession `=` 精确匹配；switchTo/detach；tmux env 回写/读取封装 |
...
- **同步路径**：改仓库 → `ptl update --all` → 模板 symlink 自动可见 → `/reload` 生效
- **跨扩展 import 依赖**：`ptl-control → _shared` 依赖共享层 symlink 布局；若模板用同名真实目录覆盖 ptl-control 而缺 `_shared`，扩展加载失败——bundled 同步保证 `_shared` 存在，文档化此依赖
...
2. 重构 `extensions/ptl-control/index.ts`（import 共享模块 + 命令面增强）
3. 重构 `extensions/ptl-communicate/index.ts`（删 registry/presence 副本与 tmux 三命令）
4. 新增/修改测试；`npm run build`（类型检查）；vitest 全绿
5. `ptl update --all` 同步共享层；验证仓库=共享层零差异
6. `/reload`（或新会话）验证：/ptl start 提示去 /control、/control start 自动命名、/control ls 在线状态、/control name 持久化
