...
| 目录 | 一行职责 | 证据入口 |
|---|---|---|
| **`src/ptl/`** | **Pi-Triple-Lite**——轻量开发/调试工具链；不实现 agent runtime，而是用 tmux 启动真实 pi 进程（模板隔离），并提供 pit CLI + 双 TUI + pit-flow 波次引擎 + hub 联邦交互桥 | `src/ptl/pit.ts`（CLI 入口，bin: pit）；`src/ptl/launcher.ts`（pi 启动参数构建）；`docs/ptl/architecture.md` |
...
- 同类扩展：`extensions/agent-lab-bidder/`（市场竞价工具 place_bid，已标 deprecated-for-bidding 保留兼容）、`extensions/pit-communicate|pit-control|pit-providers|workflow`（PTL 侧扩展，共享层 symlink 注入模板）。
...
| spec | 主题 | 状态 |
|---|---|---|
| 08-01 pit-communicate-control-consolidation | pit-communicate/pit-control 扩展整合 | 已确认 |
| 08-01 pit-update-release | 发行版更新机制 | 已确认 |
| 08-02 workflow-runtime-extension | 子项目 A：pit-flow code 节点+metrics 声明 | 已合并 |
...
```
开发者机器：PTL CLI（本机 tmux 工具）+ pit hub <…>（HTTP/SSE/WS）
   ↓
Docker Compose 联邦态：
...
- PTL 保持本机 tmux 工具不变，hub 从"程序包上传"扩展为完整联邦交互层：ComponentManifest 泛化（§5.1）、targetSlot 空位绑定（§5.2）、legalAuth 声明式登记（§5.3）、fallback_requests 回退通道 + `pit hub request/requests/respond`（§5.4）、`hub observe/debug`（§5.5）。
...
- 经济/记忆/装配（08-02/03）：市场经济体制（方案 A）三阶段 = pit-flow 运行时扩展(A) → 记忆系统(B) → 装配层(C) → 经济层(D)，economy 域=**框架**（framework-vs-construction.md 三层划分：框架/构造/平台，ptl CLI 属平台层不进容器）。
...
| 4 | **CLI 化迁移未开始** | 08-06 cli-template-migration：P1 试点（cli-dev 模板 + 通用排除机制 + 双技能 + local→root 更名） | 无 cli-dev/pit-agent 痕迹（find 无结果）；`src/ptl/shared-layer.ts` symlink 扩展注入机制仍活跃；spec 状态=设计待评审 | 低（计划内，未启动） |
