...
| **ptl-communicate** | 跨 pi 会话通信：文件邮箱 `/ptl send/ask/inbox/share` + manual/auto/hybrid 审核 + 不可变审计日志 | default factory `ptlMail`（`index.ts:115`），`main: index.ts` | PTL-symlink 仅 | **稳定期（特性冻结，仅 session_start 更新提示等小改）**；但**记忆 spec 的桥接依赖未落地**（见 §关系） | 2026-08-01 |
| **ptl-control** | 会话内管理 tmux 后台 pi 会话：`/control start/stop/ls/switch/detach/ui/name/status` | default factory `ptlControl`（`index.ts:23`） | PTL-symlink 仅 | 稳定（08-01 与 ptl-communicate 整合重构后冻结） | 2026-08-01 |
| **ptl-providers** | 统一 provider 后端：声明式 `providers.json` + 多 Key 池 + 401/403 failover + `/keys` 管理 + 动态 refreshModels | default factory `ptlProviders`（`index.ts:428`） | PTL-symlink 仅 | **活跃单点**（hook 类，唯一一次提交 2026-07-29；含双注册防护检测旧 kimi-platform/ustc-llm） | 2026-07-29 |
| **workflow** | pi 会话内 ptl-flow 接口：`/flow` 命令 + `flow_run/flow_status/flow_ls` 工具 + human-gate 通知 | default factory（`index.ts:92`） | PTL-symlink 仅 | 活跃（依赖 `ptl flow` CLI，runner.ts 全部 shell 外调，零进程内逻辑） | 2026-08-01 |
| **_shared/** | **非扩展**（注释明示"勿加 index.ts"）：paths/presence/registry/tmux-session/version-check 五个共享模块 | 无 index.ts 无 package.json | 随 symlink 进入模板目录但**不被 pi 加载** | 供 ptl-communicate / ptl-control 共用（08-01 整合时从 communicate 迁出） | 2026-08-01 |

### 精确引用

- 打包为 bundled 的 6 个目录：`/Users/anzhize/pi-platform/extensions/`（agent-lab、agent-lab-bidder、ptl-communicate、ptl-control、ptl-providers、workflow；`_shared` 不算第 7 个）。
- 加载机制（`src/ptl/shared-layer.ts`）：
  ...
  - `syncBundledExtensions` (L212-276) `ptl update --all` 覆盖式同步 + `.bundled-manifest` 剪枝（调用点 `src/ptl/cli/admin.ts:196-199`）；
...
| **ptl-communicate** | **spec 层依赖，代码层未接线**：记忆系统 spec 计划 C "传输复用 ptl-communicate + 语义桥接"（`docs/superpowers/specs/2026-08-02-memory-system-design.md:182,223`）；agent-lab 的 `CommsBridge`（`extensions/agent-lab/src/assembly/comms-bridge.ts`）实现了契约⑥⑨⑩ 但**自带 inbox.jsonl + IdentityMap**，不 import ptl-communicate；`CommsTransport` 接口（`extensions/agent-lab/src/memory/comms.ts:39-43`）**无生产实现**（scout-4 已确认仅测试 mock）→ "Task 12 ptl-communicate 接线"未落地 | comms-bridge.ts:41,132 注释反复出现 "ptl-communicate session_start 时接线"（未来时） |
| **ptl-control** | 无直接依赖；与 ptl-communicate 共享 `_shared` 模块（08-01 consolidation，plan `docs/superpowers/plans/2026-08-01-ptl-communicate-control-consolidation.md`） | `extensions/ptl-control/index.ts:13-16` |
| **ptl-providers** | 无依赖；仅防护性检测 legacy 扩展存在 | `index.ts:31-48` |
| **workflow** | 无依赖；与 PTH 的 `src/pth/workflow/`（BullMQ 编排）**同名但完全独立**（本地 ptl-flow 接口 vs 服务器编排） | runner.ts 全部外调 `ptl flow` CLI |
...
- **现状：迁移尚未开始**——6 个 bundled 扩展全部仍在打包、仍逐模板 symlink 注入；`ptl update --all` 覆盖同步照常运行。P1（cli-dev 试点 + 排除机制 + local→root 更名）进行中，P3 逐扩展迁移未动。
- 分类归属（按 CLI 迁移 spec §1 表格）：
  - 工具类：**agent-lab-bidder (place_bid)** → (b) 适配壳候选；
  - 命令/UI 类：**ptl-control (/control)**、**workflow (/flow)** → `ptl CLI --json` 候选；
  - 进程内 hook 类：**ptl-providers**（after_provider_response 401/403）→ localhost provider proxy 专项或**永久留 root**；
  - 实时推送/遥测类：**ptl-communicate**（投递）、agent-lab（遥测注入）→ 轮询 CLI / sidecar，接受延迟降级。
...
- **单仓双产品**：PTL（`ptl` CLI，`dist/ptl/pit.js`，真实 pi × tmux，本地 TUI）与 PTH（`pth` server，`dist/pth/main.js`，AgentEngine + Redis + BullMQ，HTTP/SSE/WebSocket，联邦治理）。共享 `src/shared/`（sdk-adapter / model-router / workspace / platform / observability / credential-provider）。**PTL→PTH 桥**：`ptl hub submit/run`（L28-42 图）。
- **数据流 A**（hub submit）：pack.ts 读 agent.json + skills → 手写 ustar.ts 打 tar → POST /api/v1/programs → ProgramStore（INCR 版本 + 安全解包）→ `ptl hub run` 起一次性 session → SSE 双信封 {seq,type,data} 流式回显。
- **数据流 B**（ptl-flow 波次）：createRun → execLock → 波循环（findReadyNodes by firedEpoch/consumed）→ 同波并行 spawn pi → 波末 reducer 合并 + checkpoint → human gate / edit 屏障。
...
1. 改扩展机制：`src/ptl/shared-layer.ts`（唯一真相）；`ptl update --all` 触发点在 `src/ptl/cli/admin.ts:196-199`。
2. 审 ptl-communicate 桥接缺口：`extensions/agent-lab/src/memory/comms.ts:39-43`（CommsTransport 无生产实现）→ `extensions/agent-lab/src/assembly/comms-bridge.ts`（自带 inbox，不依赖 ptl-communicate）。
