...
|---|---|---|---|
| `ptl.ts` | CLI 入口（bin: pit）：先装 node:sqlite ExperimentalWarning 过滤，再延迟加载主流程 | `main()` from `ptl/run.js`；re-export `parseArgs`/`resolveOrFail` | pit.ts:1-25 |
...
| `config.ts` | 中心配置 `pi-triple.json`（v2/v3，模板 UUID+alias，`pth.url/token` 配置，`PI_TRIPLE_HOME` 覆盖） | `PiTripleConfig`、`loadConfig`/`saveConfig`/`resolveTemplateId`/`getTemplateAlias`/`ptlHome` | config.ts:1-80+ |
...
| `picker.tsx` | 交互式模板选择器（Ink/React，`ptl start` 无参数时用） | `interactiveStart` | picker.tsx:1-162 |
| `ptl/` | CLI 子命令实现（10 文件，见 §2） | `args`/`main`/`mode`/`sessions`/`onboard`/`config-cmd`/`admin`/`route`/`agent` | — |
...
| `shared-layer.ts` | **共享扩展层**：bundled `extensions/` 逐项 symlink 注入各模板 + `.bundled-manifest` 剪枝 + `ptl update --all` 覆盖式同步 | `linkTemplateToShared`/`ensureTemplateLinks`/`installBundledExtensions`/`syncBundledExtensions`/`promoteToShared` | shared-layer.ts:1-274 |
...
| `tmux.ts` | **tmux 会话管理**：单一命名规则（`ptl-` 前缀）、env 注入（仅 `PI_*`/`AGENT_LAB_*`，`-e K=V` 禁 shell 拼接）、csi-u 配置 | `startPitSession`/`buildTmuxSessionArgs`/`listPitSessions`/`listPitPanesDetailed`/`sessionsForTenant`/`killPitSession` | tmux.ts:1-201 |
...
| `flow/` | **ptl-flow 波次引擎**（12 文件，见 §3） | `engine`/`schema`/`store`/`pm`/`commands` | — |
...
**解析**：`ptl/args.ts`（`SUBCOMMAND_COMMANDS = {template, shared, config, flow, agent, tui, hub, session, trace}`，args.ts:5）；**分发**：`ptl/run.ts` 的 `main()` switch（run.ts:127-249）。
...
**JSON 模式**：`--json` 时走 `ptl/mode.ts` 表驱动 `JSON_ROUTERS`（template/status/doctor/ls/stop/shared/flow/agent/hub/session/trace 支持；hub 仅 programs，mode.ts:27-83）。
...
**tmux 载体**（tmux.ts:136-201）：`buildTmuxSessionArgs` 用 `tmux new-session [-d] -s ptl-<name> -c <cwd> -x 200 -y 50 -e K=V -- pi <args>`——**只透传 `PI_`/`AGENT_LAB_` 前缀 env**（tmux.ts:153-157），`--` 分隔禁 shell 注入。`configureTmuxServer` 设 extended-keys csi-u（pi 官方推荐，tmux.ts:27-38）。启动后 `getPanePid` 拿 pid 写进 session-registry。

**ptl-flow 的独立 spawn 路径**（flow/pm.ts:1-59）：`makeSpawnAgent` → `spawn(pi, ["--print","--mode","json","--no-session"], {detached:true})`，prompt 经 stdin 写入，stdout 逐行解析 JSON 事件聚合 `text_delta`，超时（默认 120s）杀整个进程组。**不经过 launcher**——flow 节点的 env 由 `FlowStore` 提供（`{...process.env, ...env}`，pm.ts:30）。

---

## 4. PTL↔PTH 桥（bridge/ + hub 命令）

**入口**：`ptl/route.ts` cmdHub（route.ts:64-121）分发 9 个子命令到 bridge 模块；`HUB_COMMANDS` 常量 route.ts:27。**配置**：`ptl config set pth.url/pth.token`（或 `PTH_URL/PTH_TOKEN` env，client.ts:100-105）。
...
4. **trace provider 只读消费**：`session/trace-provider.ts` 的 bidding/machine providers 读 agent-lab DB（credit_tx、lab_events machine.transition），供 `ptl trace` 命令族（trace-provider.ts:1-99）。
...
**agent-lab 能力在 PTL 侧的面板**：`ptl tui lab`（tui-lab/）+ `ptl trace` + `ptl agent run`——全部通过上述通道消费。

---

## 6. 其他值得注意的架构点

- **命令双通道**：`commands.ts`（纯函数）+ `commands/dispatch.ts`（CLI/TUI 共用分发，进程内不安全命令 handoff 到 `ptl <子命令>` 子进程，dispatch.ts 头注释）。
- **会话恢复链**：session-registry（持久化）→ tmux 存活探测（session-state.ts 状态机）→ `ptl restore` 按注册表重建 + `--session <id>` resume 原纸带（commands.ts execStartBg、pit/sessions.ts cmdRestore:309）。
- **ptl-flow 热修改**：运行中 `set/edit` 排队 pending → 波边界停（`editing`）→ resume 校验应用（engine.ts，v2 波次；双锁 exec/mutation 在 store.ts 头注释）。
...
- 想改"扩展注入"→ 打开 `shared-layer.ts`（symlink 机制）+ `launcher.ts:119-121`（启动补链点）+ `ptl/admin.ts`（install/update 走 installBundledExtensions/syncBundledExtensions）
- 想改"PTL↔PTH 桥"→ 打开 `bridge/client.ts`（端点全集）+ `ptl/route.ts:64-121`（分发）
- 想改"ptl-flow"→ 打开 `flow/engine.ts`（executeLoop 波次循环）+ `flow/pm.ts`（spawn pi）
- 想改"会话管理"→ 打开 `tmux.ts` + `ptl/sessions.ts`
