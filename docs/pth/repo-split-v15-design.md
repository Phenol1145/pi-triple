...
> 状态：设计已批准；**Phase 0–4 全部完成**（旧仓已 archive）。
> Phase 1 报告：[phase1-deps-split-report.md](./phase1-deps-split-report.md) · Phase 2 报告：[phase2-pth-split-report.md](./phase2-pth-split-report.md) · Phase 3 报告：[phase3-ptl-split-report.md](./phase3-ptl-split-report.md) · Phase 4 报告：[phase4-final-report.md](./phase4-final-report.md)。
...
4. **Phase 3 ✅ 已完成（本地 + GitHub）**：filter-repo 拆出 `/Users/anzhize/pi-triple-ptl`；依赖切 npm 版本；framework bin wrapper 使 `npm ci` 后 `ptl` 直接可用；构建/lint/464 tests 全绿；PTL 安装不触发 PTH 源码下载。报告：`docs/pth/phase3-ptl-split-report.md`。
5. **Phase 4 ✅ 已完成**：旧仓 GitHub archive（README 置顶三仓指引）；三仓 README/CI/文档索引独立化；mailbox/dev-container 边界归属修正。报告：`docs/pth/phase4-final-report.md`。
