...
const PTH_UNIT_TESTS = [
  ...
  "test/unit/session-pool-redis.test.ts",
  "test/unit/slot-binding.test.ts",
...
const DEPS: RepoSplitRepo = {
  ...
  filterRepoPaths: ["packages/shared", "packages/infra", "test/shared-barrel.test.ts", ...DEPS_UNIT_TESTS],
...
const PTH: RepoSplitRepo = {
  ...
  filterRepoPaths: [
    ...
    "test/helpers.ts",
    "test/setup.ts",
    "test/integration/engine-lifecycle.test.ts",
    "test/integration/storage.test.ts",
    "test/ptl-kernel-bridge.test.ts",
    "test/zz-agent-dbg.test.ts",
    "packages/framework/test/bridge-client.test.ts",
    ...PTH_UNIT_TESTS,
    "docs/pth",
    "scripts/pth-cli.ts",
  ...
  notes: "bin: pth → dist/cli/pth-cli.js（根 tsc 编译；pth-console 作为库导出 launcher/commands/web）。Dockerfile.dev 为 PTH 专用 node dev 镜像。",
};
...
const PTL: RepoSplitRepo = {
  ...
  postFilterRemovePaths: [
    ...
    "test/integration/engine-lifecycle.test.ts",
    "test/integration/storage.test.ts",
    "test/unit/repo-split-manifest.test.ts",
    "packages/framework/test/bridge-client.test.ts",
  ],
  copyBoth: [
    "tsconfig.base.json",
